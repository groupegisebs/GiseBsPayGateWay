...

{
    ...
    Task RemoveForFailedPaymentAsync(
...

{
...

    {
...

        {
            ...
            _logger.LogInformation(
                ...
                "Session {SessionId} non payée (payment_status={PaymentStatus}); taxes non enregistrées pour {PaymentCode}",
...

        {
            ...
            _logger.LogInformation(
                ...
                "Paiement {PaymentCode} non confirmé (status={Status}); taxes non enregistrées",
...

        {
...

            {
                ...
                _logger.LogInformation(
                    ...
                    "PaymentIntent {PaymentIntentId} non réussi (status={Status}); taxes non enregistrées pour {PaymentCode}",
...

    {
        ...
        if (!StripePaymentVerification.IsInvoicePaid(stripeInvoice))

        {

            _logger.LogInformation(

                "Facture Stripe {InvoiceId} non payée (status={Status}); taxes non enregistrées",
...

        {
            ...
            _logger.LogInformation(
                ...
                "Paiement {PaymentCode} non confirmé (status={Status}); taxes facture non enregistrées",
...

    {
        ...
        CollectedTaxRecord? record = null;
...

        {
            ...
            record = await _db.CollectedTaxRecords

                .Include(x => x.Lines)

                .FirstOrDefaultAsync(x => x.PaymentTransactionId == paymentTransactionId, cancellationToken);

        }



        if (record is null && !string.IsNullOrWhiteSpace(paymentCode))
        ...
        if (record is null && !string.IsNullOrWhiteSpace(transactionReference))

        {
            ...
            record = await _db.CollectedTaxRecords
                ...
                .FirstOrDefaultAsync(x => x.TransactionReference == transactionReference, cancellationToken);
        ...
        _db.CollectedTaxLines.RemoveRange(record.Lines);

        _db.CollectedTaxRecords.Remove(record);
        ...
        _logger.LogInformation(
            ...
            "Taxes collectées supprimées pour {PaymentCode} après échec de paiement",
